...
class V8PreferencesScreen extends StatefulWidget {
  ...
  final String? place;
  const V8PreferencesScreen({super.key, required this.mode, required this.activity, this.place});
...
class _V8PreferencesScreenState extends State<V8PreferencesScreen> {
  ...
  Future<void> _createWithAi() async {
    ...
    try {
      ...
      await Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => _AiCreatingScreen(suggestionFuture: suggestionFuture),
        ),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final placeLabel = widget.mode == V8Mode.plan ? (widget.place ?? '') : 'Dalla tua posizione';
    return Scaffold(
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Icon(Icons.tune_rounded, color: Colors.white, size: 27),
                    ],
                  ),
                ),
                Expanded(
